...
```bash
docker run -d --name os_analytics_container -p 8080:8080 python:3.11-slim python3 -m http.server 8080
```

```bash
docker ps --filter name=os_analytics_container
```

```
CONTAINER ID   IMAGE              COMMAND                  CREATED        STATUS        PORTS                    NAMES
3852830bcf0f   python:3.11-alpine   "python3 -m http.ser…"  4 seconds ago  Up 3 seconds  0.0.0.0:8080->8080/tcp   os_analytics_container
```
...
docker exec os_analytics_container sh -c \
...
```bash
docker logs os_analytics_container
```

```
192.168.65.1 - - [18/Jun/2026 21:03:01] "GET / HTTP/1.1" 200 -
192.168.65.1 - - [18/Jun/2026 21:03:01] code 404, message File not found
192.168.65.1 - - [18/Jun/2026 21:03:01] "GET /test.txt HTTP/1.1" 404 -
```
...
```bash
docker stop os_analytics_container
```

```
os_analytics_container
```

```bash
docker ps -a --filter name=os_analytics_container
```

```
CONTAINER ID   IMAGE              COMMAND                  STATUS
3852830bcf0f   python:3.11-alpine   "python3 -m http.ser…"   Exited (137) Less than a second ago
```
